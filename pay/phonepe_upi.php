...
function phonepe_page_escape(string $value): string {
    return htmlspecialchars($value, ENT_QUOTES, 'UTF-8');
}

function phonepe_setting(mysqli $conn, string $key, string $default = ''): string {
...
$minimum = 100.0;
$maximum = 50000.0;
$payeeVpa = trim(phonepe_setting($conn, 'phonepe_upi_id'));
if ($payeeVpa === '') {
    $payeeVpa = trim(phonepe_setting($conn, 'wake_upi_id'));
}
$payeeName = trim(phonepe_setting($conn, 'phonepe_upi_name', 'Jalwa')) ?: 'Jalwa';
...
if ($error === '' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$paymentConfirmed || $orderId === '') {
    ...
    } elseif (!preg_match('/^[0-9]{20}$/', $orderId)) {
        $error = 'The payment order is invalid. Please start a new deposit.';
    } else {
        $orderStmt = $conn->prepare("SELECT shonu, motta, dinankavannuracisi FROM thevani WHERE dharavahi = ? AND balakedara = ? AND mula = ? AND sthiti = '0' LIMIT 1");
        $order = null;
        if ($orderStmt) {
            $orderStmt->bind_param('sis', $orderId, $uid, $channel);
            $orderStmt->execute();
        ...
        if (!$order) {
            $error = 'This PhonePe order was not found or has already been processed. Please start a new deposit.';
        } elseif (!$isExpired) {
            ...
            if (strlen($utr) < 6 || strlen($utr) > 80) {
                ...
                if ($duplicate) {
                    ...
                    $update = $conn->prepare("UPDATE thevani SET ullekha = ? WHERE shonu = ? AND balakedara = ? AND mula = ? AND sthiti = '0'");
                    if ($update) {
                        $orderRowId = (int)$order['shonu'];
                        $update->bind_param('siis', $utr, $orderRowId, $uid, $channel);
...
if ($error === '' && $_SERVER['REQUEST_METHOD'] !== 'POST') {
    if ($amount < $minimum || $amount > $maximum) {
        ...
        $error = 'PhonePe UPI ID is not configured yet. Please contact support.';
    }
    if ($error === '') {
        ...
        if (!$insert) {
            $error = 'Could not create the PhonePe order. Please try again.';
        } else {
...
<body>
  <main class="wrap">
    ...
    <?php if ($error !== '' && (!($paymentConfirmed && $orderId !== '') || $isExpired)): ?><div class="card"><h1>PhonePe UPI</h1><div class="error"><?= phonepe_page_escape($error) ?></div></div>
    <?php elseif ($submitted): ?><div class="card"><h1>UTR Submitted</h1><div class="success"><?= phonepe_page_escape($statusMessage) ?></div><div class="amount">₹<?= phonepe_page_escape(number_format($amount, 2)) ?></div><div class="label">Order ID</div><div class="order"><?= phonepe_page_escape($orderId) ?></div></div>
    <?php elseif ($paymentConfirmed): ?><div class="card">
      ...
      <?php if ($error !== ''): ?><div class="error"><?= phonepe_page_escape($error) ?></div><?php endif; ?>
      <p class="muted">Enter the UTR or transaction reference shown after completing the PhonePe payment.</p>
      ...
      <form method="post">
        <input type="hidden" name="payment_done" value="1"><input type="hidden" name="order_id" value="<?= phonepe_page_escape($orderId) ?>">
    ...
    <?php else: ?><div class="card">
      ...
      <div id="utrSection" class="utr-section" hidden>
        ...
        <form method="post"><input type="hidden" name="payment_done" value="1"><input type="hidden" name="order_id" value="<?= phonepe_page_escape($orderId) ?>">
          <div class="label">UTR / transaction hash</div><input type="text" name="utr" minlength="6" maxlength="80" placeholder="Enter payment reference" required>
          <button class="button" type="submit">Submit UTR</button>
        </form>
      </div>
    ...
    <script>
      (function () {
        ...
        // Keep external-app navigation behind the user’s direct tap on the Open PhonePe button. This prevents browsers from returning to Deposit or opening an empty tab when the app chooser/security check cancels the launch.
      }());
    </script><?php endif; ?>
  </main>
</body>
