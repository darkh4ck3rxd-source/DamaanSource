...
if ($error === '' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$paymentConfirmed || $orderId === '') {
        ...
        $orderStmt = $conn->prepare("SELECT shonu, motta FROM thevani WHERE dharavahi = ? AND balakedara = ? AND mula = 'PAYTM_UPI' AND sthiti = '0' LIMIT 1");
        $order = null;
        if ($orderStmt) {
