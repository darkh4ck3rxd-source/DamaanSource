...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? 'save') === 'preview_upload' && $parent && $error === '') {
    $file = $_FILES['metric_file'] ?? null;
    if (!$file || (int)($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
        $uploadError = 'Choose a readable pipe-separated file first.';
    } elseif ((int)($file['size'] ?? 0) > 2 * 1024 * 1024) {
        $uploadError = 'The upload must be 2 MB or smaller.';
    } else {
        ...
        foreach ($lines ?: [] as $lineNumber => $line) {
            ...
            if ($line === '') { continue; }
            if (!$parsed && preg_match('/^\s*level\s*\|/i', $line)) { continue; }
            $parts = array_map('trim', explode('|', $line));
            if (count($parts) !== 3 || !preg_match('/^\d+$/', $parts[0]) || (int)$parts[0] < 0 || (int)$parts[0] > 6 || !preg_match('/^\d+(?:\.\d{1,2})?$/', $parts[1]) || !preg_match('/^\d+(?:\.\d{1,2})?$/', $parts[2])) {
                $uploadError = 'Invalid row ' . ((int)$lineNumber + 1) . '. Use level|deposit_amount|commission with valid non-negative values.';
                break;
            }
            $parsed[] = ['level' => (int)$parts[0], 'deposit_amount' => round((float)$parts[1], 2), 'commission' => round((float)$parts[2], 2)];
        }
        if ($uploadError === '' && count($parsed) !== count($children)) {
            $uploadError = 'File has ' . count($parsed) . ' data rows, but the loaded editor has ' . count($children) . ' UID rows. Nothing was saved.';
        }
        if ($uploadError === '') {
            foreach ($children as $index => $child) {
                $uploadPreview[(int)$child['id']] = $parsed[$index];
            }
            $uploadNotice = count($parsed) . ' rows mapped to the visible UID order. Review the fields below, then press Save Changes.';
        }
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? 'save') === 'save' && $parent && $error === '') {
...
<?php if ($uploadNotice): ?><div class="notice"><?= htmlspecialchars($uploadNotice, ENT_QUOTES, 'UTF-8') ?></div><?php endif; ?>
<?php if ($uploadError): ?><div class="error"><?= htmlspecialchars($uploadError, ENT_QUOTES, 'UTF-8') ?></div><?php endif; ?>
...
<form method="post" enctype="multipart/form-data" class="demo-box"><input type="hidden" name="parent" value="<?= $parentId ?>"><input type="hidden" name="metric_date" value="<?= htmlspecialchars($metricDate, ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="action" value="preview_upload"><h4>Upload UID values for preview</h4><p class="hint">One data line maps to one visible UID row from top to bottom. Header is optional. Format: <code>level|deposit_amount|commission</code></p><input type="file" name="metric_file" accept=".txt,.csv,text/plain,text/csv" required><button type="submit">Preview file mapping</button></form><form method="post"><input type="hidden" name="parent" value="<?= $parentId ?>"><input type="hidden" name="metric_date" value="<?= htmlspecialchars($metricDate, ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="action" value="save"><h3>Summary values for <?= htmlspecialchars($metricDate, ENT_QUOTES, 'UTF-8') ?></h3><p class="hint">These values override the six number cards shown on the user’s Subordinate Data page for the selected date.</p><div class="summary-grid">
