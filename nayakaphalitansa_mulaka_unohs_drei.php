...
// 3. use API only if manual not there and useAPI is true
if ($manualNumber === null && $useAPI) {
    ...
    $data = json_encode([
        ...
        "typeId" => 2,
        "language" => 0,
        "random" => "41ee0335f94b4c2793f294879bb0c614",
        "signature" => "9AF59C9894939C4B530CD9D9EF8A77F6",
        "timestamp" => time()
    ]);
...
// **4. Check hastacalita_phalitansa_drei for override
$hastedsankhye = null;
...
//** rest of code after data fetch begins here using the determined $defaultNumber variable...
	...
	if($samasyesreni['atadaaidi'] != null){
		...
		if($gadhipathulidhadi >= 1){
			...
			$tathya = mysqli_query($conn,"INSERT INTO `gellaluhogiondu_phalitansa_drei` (`kalaparichaya`,`bele`,`phalitansa`,`banna`,`phalitansadaprakara`,`dinankavannuracisi`) VALUES ('".$samasyesreni['atadaaidi']."','".$yadrcchikasankhye."','".$kadimesucyanka."','".$banna."','shonu','".$dinanka."')");
			...
			if($kadimesucyanka == 6){
				$nabikarana = "UPDATE bajikattuttate_drei set phalaphala = 'gagner', sesabida = ROUND(sesabida * 2, 2), ergebnis = '".$kadimesucyanka."', zufallig = '".$yadrcchikasankankhye."', tiarikala = '".$dinanka."' WHERE kalaparichaya = '".$samasyesreni['atadaaidi']."' AND ojana = '10'";
				$conn->query($nabikarana);
				$nabikarana = "UPDATE shonu_kaichila
				INNER JOIN (
					SELECT byabaharkarta, SUM(sesabida) AS total_paid
